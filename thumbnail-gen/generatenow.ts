...
const generateScreenshot = async (
  ...
  while (retryCount <= maxRetries) {
    try {
      console.log(
        `Generating screenshot for: ${toolName} (${url}) - attempt ${
          retryCount + 1
        }/${maxRetries + 1}`
      );
      ...
      if (retryCount < maxRetries) {
        ...
        console.error(
          `Failed after ${
            maxRetries + 1
          } attempts to generate screenshot for ${url}`
        );
        return false;
      }
    }
...
const dismissPopups = async (page: any): Promise<void> => {
  try {
    ...
    const prioritySelectors = [
      ...
      ".js-cookie-consent-dismiss",
      ".js-consent-banner-dismiss",
    ...
    for (const selector of prioritySelectors) {
      try {
        ...
        if (elements.length > 0) {
          console.log(
            `Found ${elements.length} priority elements with selector: ${selector}`
          );
          ...
          for (const element of elements) {
            ...
            if (
              ...
              console.log(
                `Skipping suspicious button with text: "${elementText}"`
              );
              continue;
            }
            ...
            if (isVisible) {
              ...
              await element
                .click()
                .catch(() =>
                  console.log(`Couldn't click priority element: ${selector}`)
                );
              console.log(
                `Clicked priority element: ${selector} with text: "${elementText}"`
              );
              await new Promise((resolve) => setTimeout(resolve, 500));
              ...
              if (!stillVisible) {
                console.log(
                  `Priority element ${selector} was successfully dismissed`
                );
        ...
        console.log(
          `Error with priority selector ${selector}:`,
    ...
    try {
      await page.evaluate(() => {
        ...
        for (const el of allElements) {
          ...
          const shouldClick = buttonTextsToClick.some((text) =>
            elText.includes(text)
          );
          const shouldAvoid = textsToAvoid.some((text) =>
            elText.includes(text)
          );
      ...
      if (page.url().includes("github.com")) {
        ...
        await page.evaluate(() => {
          ...
          const dismissButtons = [
            ...
            ...Array.from(
              document.querySelectorAll(".js-consent-banner-dismiss")
            ),
            ...Array.from(
              document.querySelectorAll(".js-cookie-consent-dismiss")
            ),
            // Close buttons by dataset attributes
            ...Array.from(
              document.querySelectorAll(
                '[data-action="consent-banner#dismiss"]'
              )
            ),
          ];
...
const updateToolsFile = (toolId: string, newImagePath: string): boolean => {
  try {
    ...
    if (updatedContent === toolsContent) {
      console.warn(
        `Found tool ID '${toolId}' but couldn't update the image path`
      );
      return false;
    }
...
const processTools = async (toolIds: string[]): Promise<void> => {
  ...
  const toolsNeedingScreenshots = toolsToProcess.filter((tool) => {
    ...
    if (hasImage) {
      ...
      console.log(
        `🔥 ${tool.name} (ID: ${tool.id}) - Image missing, will process`
      );
      return true;
    }
  });
  ...
  existingFiles.forEach((file) => {
    const baseName = file.replace(".png", "");
    const isInToolsList = toolsToProcess.some(
      (tool) => generateImageFilename(tool.name, tool.id) === baseName
    );
    if (!isInToolsList) {
      console.log(
        `❌ ${file} - File exists but tool is not in the list you provided`
      );
    }
  });
    ...
    try {
      ...
      if (success) {
        ...
        if (updateSuccess) {
          ...
          console.error(`If it is a test, you can ignore the error above.`);
        }
      } else {
        failCount++;
